...
Our contributions are **(1)** a target-sensitive identification boundary, **(2)** conditional certification rules with explicit failure modes, and **(3)** an executable local audit that separates timing, coverage, and statistical validity. The main empirical result is a synthetic robustness failure and its correction, not a measured LLM tendency. The key threat to validity is transport: the deployment reference is supplied to the mathematics, not learned from the local benchmark. Earlier theory and CPU results are carried forward. This execution reproduces the saved experiments and audits their records; it adds no model evidence.

<!-- MAIN PAGE BREAK -->

## 2. Related Work

**Sandbox detection.** The inherited project attributes an interactive distinguishing game and an external-consequence witness to Penumbra [1, Sections 2 and 5]. Its source attachment is unavailable here, so this comparison remains unverified. Our question is whether detectable differences leave a specified action probability unidentified. A coherent local replica refutes an unrestricted claim that containment always forces detectable distortion.
...
The supplied CPU results were reproduced before changes. Seven regenerated files match their originals byte for byte. Original records and new reproduction outputs remain separate. Appendix C gives replication and missingness rules; this retrospective CPU work is not public preregistration.
...
No evaluated LLM request was made. Two saved smoke tests each completed 12 mock episodes and 36 mock requests; all are excluded from model claims. Inherited mock logs remain separate. Exact model identifiers, endpoint permission, current prices, and a spending budget are absent. Provider compatibility still requires a separately logged authorized pilot.
...
The companion ZIP contains source documents, code, immutable records, reproduced results, figures, verification files, and hashes. No public repository was created. Author identity, affiliation, artifact location, and human ownership of the writing remain submission gates. Limitations and LLM usage follow.

## References

**[1]** Penumbra: Sandboxing Autonomous Agents by Measuring, Not Assuming, Indistinguishability. Inherited project reference to paper (1).pdf, Sections 2 and 5. Source attachment unavailable in this workspace; author, date, publication status, and attribution require verification.
...
**[3]** Sanyu Rajakumar and Rudolf Laine. Eliciting Situational Awareness in Large Language Models Through Fine-Tuning and In-Context Mechanisms. Inherited project reference, Section 3.2. Source attachment unavailable here; metadata and comparison require verification.
...
The original CPU configuration was reproduced into results/reproduced before edits. All seven files matched byte for byte: 960 episode records, 3,000 sampling count vectors, summaries, population checks, exact crossing fractions, and totals. The sampling seed is 620231. Both inherited certificate files passed the independent rational checker: 38 cases and 2,031 constraints. A saved-record audit separately checked the reported counts, estimates, and immutable hashes. These checks are reproduction, not new independent observations.

The future model ledger retains planned cells, provider attempts, visible answers, returned identifiers, token usage, explicit refusals, truncations, parsing errors, and resource stops. Failed reports preserve completed actions. For k observed shortcuts and m missing actions among n attempted episodes, the realized attempted-sample rate lies in [k/n,(k+m)/n]. The analyzer also reports bounds over all planned episodes, including unstarted cases, and separate probability-report denominators. Classification uses fixed orientation, half credit at 0.5, paired label swaps, and Holm correction over four reserved comparisons. Missingness bounds are not deployment guarantees.

START_HERE.md gives reproduction commands; existing output directories are never overwritten. Original and revised transport smoke tests are separate mock cohorts. Seventeen unit tests passed, including exhaustive causal-prefix checks and offline refusal, truncation, missingness, and label-randomization controls. No provider inference or spending occurred. Real execution requires exact identifiers, current prices, access, a separate pilot, and a new source/configuration freeze. Bibliographic comparisons are inherited; source attachments [1,3] are absent and external source review was not performed.
...
This collaboration draft and implementation used ChatGPT and Codex assistance for theory exploration, editing, code, and checking. Codex reproduced the CPU results, reran certificate checkers, added offline failure-accounting tests, and prepared this release. No evaluated LLM experiment was run. These checks are not independent human theorem or source review. The inherited author name remains unconfirmed. Human authors must own and review the final writing, claims, references, affiliation, and disclosure before submission; the supplied template encourages a report primarily written by the team.
