...
Current execution status (14 September 2026): the CPU results were reproduced before edits and all seven files matched byte for byte; 17 tests now pass. The revised manuscript source is ready, but rebuilding its DOCX/PDF is blocked by unavailable document/plot tools and a pending exception for isolated dependency downloads. The supplied top-level PDF/DOCX/Markdown set is unchanged; `submission_revision.md` is the revised text export. Read `EXECUTION_REPORT.md` and current `verification/final_checks.json` before treating historical render checks as current.
...
The demonstrated output paths now exist. Future reproduction must choose a fresh output directory, such as `results/reproduced_next`; the runner rejects nonempty directories to preserve evidence. Do not resume a frozen mock or model directory after source changes. New source text can be exported without rendering dependencies using `python scripts/export_markdown.py --out submission_revision.md`.
