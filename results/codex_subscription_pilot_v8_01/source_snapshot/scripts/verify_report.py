...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--pdf',type=Path,default=ROOT/'submission_report.pdf');ap.add_argument('--out',type=Path,default=ROOT/'verification/report_checks.json')
    ap.add_argument('--docx',type=Path,default=ROOT/'submission_report.docx')
    ...
    with fitz.open(a.pdf) as d:
        ...
        rendered_abstract=first.split('Abstract',1)[1].split('1. Introduction',1)[0]
    ...
    assert len(doc.tables)==4 and len(doc.inline_shapes)==2
