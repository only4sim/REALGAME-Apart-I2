...
- Episodes and analyses: runs.jsonl, episodes/, analysis.json and analysis_rechecked.json.
...
- Code diff and tests: verification/subscription_v8/adapter_code.diff, adapter_code_manifest.json and unit_tests_*.txt.
- Audit: verification/subscription_v8/final_payload_audit.json and current_checks.json.

The revised paper/submission_blocks.json and submission_revision_v8_unrendered.md include these scoped results and preserve the 150-word abstract, eight main source groups, limitations/dual-use, LLM usage statement and exactly four one-month extensions. Rendering remains unavailable: docx, lxml, fitz, matplotlib, Pandoc, LibreOffice and Old Standard are absent. The verified inherited v7 PDF/DOCX are preserved byte-for-byte and do not contain these new results. Source groups do not prove pagination; the v8 source has not passed a new eight-page rendering or page inspection.
