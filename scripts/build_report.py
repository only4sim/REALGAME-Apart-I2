...
def build(blocks_path,out):
    ...
    abstracts=[b['text'] for page in x['main_pages'] for b in page if b['type']=='abstract']
    assert abstracts==[x['abstract']] and len(x['abstract'].split())==150
    assert len(x['main_pages'])==8
    ...
    for page in x['main_pages']:
        for b in page:
            if b['type'] in ('author','event'):md.extend([b['text'],''])
    equation_count=0
